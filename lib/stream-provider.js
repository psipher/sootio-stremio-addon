...
const RESOLVE_FAIL_TTL_MS = parseInt(process.env.RESOLVE_FAIL_TTL_MS || '0', 10); // 0ms (do not cache failures)
...
function backgroundPreResolve(streams, provider) {
  ...
  for (const stream of resolvable) {
    try {
      ...
      if (!match) continue;
      // Strip query params (?t=... etc.) so the cache key matches the on-demand resolver path
      const encodedUrl = match[1].split('?')[0];
          ...
          addStreamTask(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-4khdhub`, config, () =>
                ...
                backgroundPreResolve(wrapped, '4KHDHub');
                return wrapped;
              }),
          ...
          addStreamTask(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-hdhub4u`, config, () =>
                ...
                const wrapped = wrapHttpStreamsWithResolver(streams, config.host);
                backgroundPreResolve(wrapped, 'HDHub4u');
                return wrapped;
              }),
          ...
          addStreamTask(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-cinedoze`, config, () =>
                ...
                const wrapped = wrapHttpStreamsWithResolver(streams, config.host);
                backgroundPreResolve(wrapped, 'CineDoze');
                return wrapped;
              }),
          ...
          addStreamTask(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-uhdmovies`, config, () =>
                ...
                const wrapped = wrapHttpStreamsWithResolver(streams, config.host);
                backgroundPreResolve(wrapped, 'UHDMovies');
                return wrapped;
              }),
          ...
          addStreamTask(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-moviesdrive`, config, () =>
                ...
                const wrapped = wrapHttpStreamsWithResolver(streams, config.host);
                backgroundPreResolve(wrapped, 'MoviesDrive');
                return wrapped;
              }),
...
async function resolveUrl(debridProvider, debridApiKey, itemId, hostUrl, clientIp, config = {}) {
  ...
  const performResolve = async () => {
    try {
      ...
      return resolvedUrl;
        ...
        const isUHDMoviesUrl = hostUrl.includes('driveleech') ||
          hostUrl.includes('driveseed') ||
          hostUrl.includes('unblockedgames') ||
          hostUrl.includes('creativeexpressionsblog') ||
          hostUrl.includes('examzculture') ||
          hostUrl.includes('sid=');
        if (isUHDMoviesUrl) {
          const { resolveUHDMoviesUrl } = await import('./uhdmovies/resolvers/url-resolver.js');
          const res = await resolveUHDMoviesUrl(hostUrl);
          return typeof res === 'object' && res ? res.url : res;
        }
        return resolveHttpStreamUrl(hostUrl);
      } else {
  ...
  if (useResolveCache && resolved) storeResolveCache(cacheKey, resolved);
  return resolved;
}
...
export function looksLikeAsianCreditName(name) {
  if (!name || typeof name !== 'string') return false;
  const cleaned = name.trim();
  // Standard CJK script check
  if (/[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uac00-\ud7af]/.test(cleaned)) return true;
  // Common Korean / Asian romanized naming patterns (e.g. "Ji Sung", "Park Hee-soon", "Lee Han-young")
  const parts = cleaned.split(/\s+/);
  if (parts.length >= 2 && parts.length <= 4) {
    const asianSurnames = new Set(['Lee', 'Park', 'Kim', 'Choi', 'Jung', 'Kang', 'Cho', 'Yoon', 'Jang', 'Lim', 'Han', 'Oh', 'Seo', 'Shin', 'Kwon', 'Hwang', 'Ahn', 'Song', 'Ryu', 'Hong', 'Goo', 'Yang', 'Son', 'Heo', 'Yoo', 'Bae', 'Won', 'Ji']);
    if (asianSurnames.has(parts[0]) || asianSurnames.has(parts[parts.length - 1])) return true;
  }
  return false;
}

export function hasAsianScriptSignals(meta) {
  if (!meta) return false;
  const fields = [meta.name, meta.original_title, meta.description, ...(meta.alternativeTitles || [])];
  return fields.some(f => typeof f === 'string' && /[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uac00-\ud7af]/.test(f));
}

export function hasLikelyAsianCredits(meta) {
  if (!meta) return false;
  const castAndCrew = [...(meta.cast || []), ...(meta.director || [])];
  if (castAndCrew.some(person => looksLikeAsianCreditName(person))) return true;
  return false;
}

export function shouldUseMkvDramaForTitle(meta) {
  if (!meta) return false;
  if (process.env.MKVDRAMA_ASIAN_ONLY === 'false') return true;
  return hasAsianScriptSignals(meta) || hasLikelyAsianCredits(meta);
}
