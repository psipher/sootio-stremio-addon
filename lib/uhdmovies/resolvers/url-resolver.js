...
export async function resolveUHDMoviesUrl(sidUrl) {
  try {
    ...
    // Filter out raw googleusercontent links - they lack 206 support and expire quickly.
    // EXCEPTION: cdn.video-gen.xyz URLs are driveseed Instant Download CDN links that
    // redirect via video-seed.dev → googleusercontent — the player handles the redirect.
    if (finalUrl && finalUrl.includes('googleusercontent.com') && !finalUrl.includes('cdn.video-gen.xyz')) {
      console.log('[UHDMOVIES-RESOLVE] Filtering out raw googleusercontent.com link (no 206 support)');
      return null;
    }
