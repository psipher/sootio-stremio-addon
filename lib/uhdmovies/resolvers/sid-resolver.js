...
import { makeCfProxyRequest, CF_PROXY_HOSTS, CF_PROXY_URL } from '../../http-streams/utils/http.js';
...
async function fetchSidPage(url, options = {}, sessionFn) {
  ...
  if (CF_PROXY_URL && CF_PROXY_HOSTS.has(hostname)) {
    try {
      ...
      const proxyRes = await makeCfProxyRequest(url, options);
      ...
      console.log(`  [SID-PROXY] Proxy returned HTTP ${proxyRes?.statusCode}, falling back to direct session`);
    } catch (err) {
      console.log(`  [SID-PROXY] Proxy error: ${err.message}, falling back to direct session`);
    }
  }
  return sessionFn();
}
...
export async function resolveSidToDriveleech(sidUrl) {
  ...
  const resolverPromise = (async () => {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      const responseStep0 = await requestWithTimeout(() => fetchSidPage(sidUrl, { method: 'GET' }, () => session.get(sidUrl)));
