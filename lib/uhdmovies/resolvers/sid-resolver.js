...
import { extractDirectUrlFromGateway } from '../../http-streams/utils/direct-extractors.js';
...
async function fetchSidPage(url, options = {}, sessionFn, jar = null) {
  ...
  const proxyUrl = process.env.CF_PROXY_URL || CF_PROXY_URL;
  if (proxyUrl && CF_PROXY_HOSTS.has(hostname)) {
    try {
  ...
  session.get = async (url, options = {}) => {
    ...
    if (response.headers && response.headers['set-cookie']) {
      for (const cookie of response.headers['set-cookie']) {
        try { await jar.setCookie(cookie, url); } catch (e) {}
      }
    }
    return response;
  };
...
export async function resolveSidToDriveleech(sidUrl) {
  ...
  const resolverPromise = (async () => {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      // Step 6: Extract driveleech URL from meta refresh tag or direct gateway response
      const directExtracted = extractDirectUrlFromGateway(finalResponse.data || '');
      if (directExtracted) {
        console.log(`  [SID] SUCCESS! Extracted direct link from gateway response: ${directExtracted}`);
