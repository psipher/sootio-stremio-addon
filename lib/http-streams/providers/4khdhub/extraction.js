...
import { makeRequest, makeImpitRequest, makeCfProxyRequest, CF_PROXY_HOSTS, CF_PROXY_URL } from '../../utils/http.js';
...
const DEAD_HUBCLOUD_DOMAINS = new Set([
    ...
    'hubcloud.me',
    'hubcloud.xyz',
    // Confirmed 100% blocked on Vercel IPs (CF WAF IP-reputation block, 2026-07-27 diagnostic):
    'hubcloud.foo'
]);
...
export async function extractHubCloudLinks(url, referer) {
    ...
    // ── CF Worker Proxy (first attempt for CF-WAF-blocked hosts) ────────────────
    // When CF_PROXY_URL is set, route requests for known Vercel-blocked hosts through
    // the CF Worker proxy. The Worker uses Cloudflare's own egress IPs, so CF WAF
    // IP-reputation blocks (which target Vercel ranges) don't fire.
    // On success we inject the proxy response into the makeRequest chain below so
    // ALL existing link-extraction logic runs unchanged.
    let initialResponseOverride = null;
    if (CF_PROXY_URL && CF_PROXY_HOSTS.has(new URL(url).hostname.toLowerCase())) {
        try {
            const proxyRes = await makeCfProxyRequest(url, { parseHTML: true });
            if (proxyRes && !isCloudflareChallenge(proxyRes.body, proxyRes.statusCode)) {
                console.log(`[HubCloud] CF proxy bypass successful for ${url} (HTTP ${proxyRes.statusCode})`);
                initialResponseOverride = proxyRes;
            } else if (proxyRes) {
                console.log(`[HubCloud] CF proxy still got CF challenge for ${url}, falling through to direct`);
            }
        } catch (proxyErr) {
            console.log(`[HubCloud] CF proxy attempt failed for ${url}: ${proxyErr.message}, falling through to direct`);
        }
    }
    // ─────────────────────────────────────────────────────────────────────────────

    return (initialResponseOverride
        ? Promise.resolve(initialResponseOverride)
        : makeRequest(url, { parseHTML: true, headers: activeHeaders || undefined, disableProxy: shouldBypassHttpProxy(url), forceProxy: activeForceProxy })
    )
        .then(async response => {
            ...
            if (isCloudflareChallenge(response.body, response.statusCode)) {
                ...
                if (FLARESOLVERR_URL) {
                    console.error(`[HubCloud] CF challenge, using FlareSolverr: status=${response.statusCode || 'n/a'} markers=${markers.join('|') || 'none'} url=${url}`);
                } else {
                    console.log(`[HubCloud] CF challenge (no FlareSolverr configured), going straight to Impit TLS bypass: markers=${markers.join('|') || 'none'} url=${url}`);
                }
    ...
    return getCachedCfCookies(domain).then(cachedCf => {
        ...
        return makeRequest(url, { parseHTML: true, signal, headers, disableProxy: shouldBypassHttpProxy(url) })
            ...
            if (isCloudflareChallenge(response.body, response.statusCode)) {
                ...
                const markers = getCloudflareMarkers(response.body);
                if (FLARESOLVERR_URL) {
                    console.error(`[HubDrive] CF challenge, using FlareSolverr: status=${response.statusCode || 'n/a'} markers=${markers.join('|') || 'none'} url=${url}`);
                } else {
                    console.log(`[HubDrive] CF challenge (no FlareSolverr), trying Impit TLS bypass: markers=${markers.join('|') || 'none'} url=${url}`);
                }
