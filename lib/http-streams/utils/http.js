...
export async function makeCfProxyRequest(url, options = {}) {
    ...
    const proxyToken = process.env.CF_PROXY_TOKEN || CF_PROXY_TOKEN;
    if (!proxyUrl || !proxyToken) {
        console.log(`[CF-PROXY] Missing CF_PROXY_URL or CF_PROXY_TOKEN at runtime: url=${proxyUrl ? 'set' : 'EMPTY'}, token=${proxyToken ? 'set' : 'EMPTY'}`);
        return null;
    }
    try {
